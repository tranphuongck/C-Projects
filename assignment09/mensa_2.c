...
Node* append_list(Node* list, String value) {
    // todo: implement (d)
	if(list == NULL){ //prüft ob list element hat
		return new_node(value, NULL);
	}else{
		Node* node = list;
		while(node->next != NULL){ 
			node = node->next;      //liest weiter bis Ende der List
		}
...
void print_situation(void) {
    // todo: implement (e)
	prints("fertige Essen: ");
	println_list(food);
	printf("nächster Essenswunsch: %s", students->value);
...
void finish(void) {
    // todo: implement (f)
	printsln("Fertig für heute. Die Mensa schließt");
...
void run_mensa(void) {
    ...
    food = NULL;
	for (int i = 0; i < 5; i++) {
		food = new_node(menu[i_rnd(menu_count)], food);
	}
    ...
    students = NULL;
	for (int i = 0; i < 3; i++) {
		students = new_node(menu[i_rnd(menu_count)], students);
	}
    ...
    while ((i = i_input()) >= -1) {
		...
		String s = students->value;
		if (i == -1) {
			printf("%s ist nicht da? Schade.\n", s);
			reputation -= 2;
		} else {
			String fd = get_list(food, i);
			// Wenn ein Essenswunsch erfüllt wurde
			if(s_equals(fd, s)){
				printsln("Vielen Dank! Ich liebe die Mensa!");
				food = remove_list(food, i);
				reputation += 1; //steigt die Reputation um eins.
				students = append_list(students, menu[i_rnd(menu_count)]); //kommt ein weiterer Studierender dazu
		...
		//wenn es keinen Studierender mehr gibt, ist es fertig.
