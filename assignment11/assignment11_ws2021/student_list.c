...
bool poor_student(void* element, int i, void* x) {
    // todo: implement
    Student* stud = (Student*) element; //cast void* element to Student*, to get access debts of Student
...
bool mat_nr_less_digits_than(void* element, int i, void* x) {
    ...
    Student* s = (Student*)element; //cast void* element to Student*, to get access all attributes of Student
    int* a = (int*)x; //Cast void* to double* to get parameter for comparing
	int counter = 0; //a variable to count
	for(int i = s->mat_nr; i > 0; i = i / 10 )//i take mat_nr every time i/10 we count one more, until i = 0.
		counter++; //count add one more until i = 0
    return (counter < (*a)); //return true if real number of digits(counter) < given number of digits
}
...
void add_debts(void* state, void* element, int index) {
    // todo: explain each line
    double* a = (double*)state; //Cast void* to double* to get parameter to caculate
    Student* s = (Student*)element; //cast void* element to Student*, to get access all attributes of Student
    *a = *a + s->debts; //add debt from void* state to void* element
}
...
void student_stats(void* state, void* element, int index) {
    // todo: explain each line
    Student_stats** s = (Student_stats**) state; //cast pointer from state to access respective attribute of Student 
    Student* e = (Student*) element; // Cast void* to double* to get all attributes of Student
    if(*s == NULL){ // wenn es keinen Student
		*s = xmalloc(sizeof(Student_stats)); //allokiert s eine Speichen
		(*s)->n = 0; //assign number of Students als 0
		(*s)->debts = 0.0; //assign debts als 0.0
	}
	(*s)->n++; // increase the number of Students
	(*s)->debts += e->debts; //sum up the debts of every Student
...
int main(void) {
    ...
    printf("first student with mat_nr digit count less than max_digits:\n");
    if(found_student != NULL){
        String s = student_to_string(found_student);
        printsln(s);
        free(s);
...
/*
 Die Vorteile von Zeigerlisten:
 +) It's practical. We can cast any attributes in a list
 +) Useful when there are many struct in Code

Yes, because we can access attributes we want without given data type but it's maybe computer in many situtation cannot understand which varable we want to use.
*/